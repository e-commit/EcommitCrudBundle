...
* La méthode `createDisplaySettingsForm` de `Crud` est maintenant protégée.
...
* Les données membres publiques de `CrudColumn` n'existent plus (`id`, `alias`, `aliasSearch`, `aliasSort`, `label`, `sortable`, `defaultDisplayed`). Utiliser à la place les fonctions getters (modificaton des valeurs impossible).
