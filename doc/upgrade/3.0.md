...
* Les méthodes suivantes de `Crud` sont supprimées:
    * `getSearcherForm`. Elle est remplacée par la méthode `getSearchForm`. Cette nouvelle méthode retourne :
        * Soit un objet `SearchFormBuilder`, avant que la méthode `createView` soit appelée
        * Soit un objet `FormView`, après que la méthode `createView` soit appelée
    * `setUseDbal`
...
* La méthode `createDisplaySettingsForm` est maintenant protégée.
* La méthode `getDefaultSense` est renommée en `getDefaultSortDirection`
* La méthode `raz` est renommée en `reset`
* La méthode `razSort` est renommée en `resetSort`
* La méthode `processForm` est renommée en `processSearchForm`
* La méthode `clearTemplate` est renommée en `createView`
...
* Ne plus appeler la méthode `init` de `Crud` dans a méthode `createCrud` des contrôleurs
...
* La signature de la fonction Twig `paginator_links` est modifiée:
    ...
    * Maintenant: `paginator_links(PaginatorInterface $paginator, string $routeName, array $routeParams = [], array $options = [])`
